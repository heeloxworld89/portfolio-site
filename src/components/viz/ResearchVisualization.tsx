 ...
 *
 * The log is verbatim from the preprint (Appendix E.2). Correction-rate phases
 * are from Table S19 (CNN, 40% noise, 5 seeds): peak 4.2/epoch, mean 1.35 over
 * epochs 0–50, zero from epoch 50 to 200. The curve between those points is drawn.
 */

// Correction rate per epoch — phase values from Table S19; the curve between them is drawn.
const baldwin = [
...
# that has already finished dying.`}</span></code>
          ...
          <p className="rvz-caption">
            In the paper’s words, standard backprop provides{' '}
...
[node_13] Goodness: 0.04 -> `}<span className="crit">CRITICAL</span>{`
   ...
   > Trigger: Loss stagnation gate open
              (12 steps, no global improvement).
   > Action: `}<span className="hl">AUTONOMOUS NEUROGENESIS</span>{`
             (Kaiming Reinitialization)
   > Constraint: Zero-sum mean-subtraction applied.
                 ||DeltaW|| <= 10%
--- VERIFICATION FORWARD PASS ---
> Global Loss Before: 1.842
> Global Loss After:  1.791
> Result: `}<span className="ok">ACCEPTED</span>{`. Node 13 successfully
          re-integrated.`}</code>
          </div>

          <p className="rvz-caption">
            Name, score, diagnosis, treatment, bound, and a{' '}
            <strong>verification pass that can reject its own repair</strong>. A single 200-epoch DAG run emitted up to{' '}
      ...
      <div className="rvz-chart">
        ...
        <p className="rvz-chart-sub">
          ...
          the network structurally adapted rather than being permanently propped up — consistent with the paper’s
          stability analysis. CNN, 40% label noise, mean of 5 seeds (Table S19).
        </p>
        ...
        <div className="rvz-phases">
          <div className="rvz-phase">
            ...
            <div className="rvz-phase-v">4.2 peak · 1.35 mean</div>
            <div className="rvz-phase-t">All structural adaptation happens in this window.</div>
          </div>
          <div className="rvz-phase">
